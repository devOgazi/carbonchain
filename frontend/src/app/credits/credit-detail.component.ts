...
@Component({
  ...
  template: `
    <div class="credit-detail">
      @if (loading()) {
        ...
        <div class="header">
          <h1>Credit <span class="mono">{{ credit()!.id | slice:0:16 }}…</span></h1>
          <span class="badge" [class]="'badge-' + credit()!.status.toLowerCase()">{{ credit()!.status }}</span>
        </div>

        <section class="card">
          <h2>Metadata</h2>
          <dl>
            <dt>Project</dt><dd>{{ credit()!.project_id }}</dd>
            <dt>Issuer</dt><dd class="mono">{{ credit()!.issuer }}</dd>
            <dt>Vintage Year</dt><dd>{{ credit()!.vintage_year }}</dd>
            <dt>Methodology</dt><dd>{{ credit()!.methodology }}</dd>
            <dt>Geography</dt><dd>{{ credit()!.geography }}</dd>
            <dt>Tonnes</dt><dd>{{ formatTonnes(credit()!.tonnes) }}</dd>
            <dt>Issued At</dt><dd>{{ credit()!.issued_at | date:'medium' }}</dd>
            <dt>IPFS</dt>
            <dd>
              <a [href]="'https://ipfs.io/ipfs/' + credit()!.ipfs_hash" target="_blank" rel="noopener">
        ...
        <section class="card">
          ...
          @if (mrvHistory().length === 0) {
            ...
            <table class="mrv-table">
              <thead><tr><th>Date</th><th>Tonnes Sequestered</th><th>Oracle</th><th>Anomaly</th></tr></thead>
              <tbody>
        ...
        @if (isOwner()) {
          <div class="actions">
            <button class="btn btn-danger" (click)="retire()" [disabled]="credit()!.status !== 'Active'">
              Retire Credit
            </button>
            <button class="btn btn-primary" (click)="sell()" [disabled]="credit()!.status !== 'Active'">
              Sell Credit
            </button>
          </div>
        }
  ...
  `,
  styles: [`
    .credit-detail { max-width: 800px; margin: 0 auto; }
    ...
    dl { display: grid; grid-template-columns: 140px 1fr; gap: 0.4rem 1rem; font-size: 0.9rem; }
    ...
    .btn { padding: 0.5rem 1.25rem; border-radius: 6px; cursor: pointer; border: none; font-size: 0.9rem; }
